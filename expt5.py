...
def stdev(numbers):
    if len(numbers) == 1:
        return 1.0   # safe value for small dataset

    avg = mean(numbers)
    variance = sum([pow(x - avg, 2) for x in numbers]) / float(len(numbers) - 1)

    if variance == 0:
        return 1.0   # avoid zero standard deviation
...
def calculateclassprobabilities(summaries, inputvector):
    ...
    for classvalue, classsummaries in summaries.items():
        ...
        for i in range(len(classsummaries)):
            mean1, stdev1 = classsummaries[i]
            x = inputvector[i]
            probabilities[classvalue] *= calculateprobability(x, mean1, stdev1)
